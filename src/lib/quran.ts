export const SURAHS = [
  { id: 1, name: "Al-Fatihah" }, { id: 2, name: "Al-Baqarah" }, { id: 3, name: "Al-Imran" },
  ...
  { id: 19, name: "Maryam" }, { id: 20, name: "Ta-Ha" }, { id: 21, name: "Al-Anbiya" },
  ...
  { id: 34, name: "Saba" }, { id: 35, name: "Fatir" }, { id: 36, name: "Ya-Sin" },
...
export const findSurah = (input: string | number) => {
  const cleanInput = input.toString().trim().toLowerCase();
  
  // Try ID first
  const byId = SURAHS.find(s => s.id === Number(cleanInput));
  if (byId) return byId;

  // Fuzzy match logic: remove "Al-", spaces, and dashes
  const sanitize = (str: string) => str.toLowerCase().replace(/^al-/, '').replace(/[\s-]/g, '');
  const sanitizedInput = sanitize(cleanInput);

  return SURAHS.find(s => sanitize(s.name) === sanitizedInput || sanitize(s.name).includes(sanitizedInput));
};